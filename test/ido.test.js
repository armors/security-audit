...
contract("IDO", ([owner, admin, user1, user2, user3, user4, user5, user6]) => {
    ...
    describe("Test buyQuota",function(){
        ...
        it("Test buyQuota More than MaxPerUser.", async function () {
            let dt = await getDT()
            await testAddPair(this.basic,this.depToken1,dt+50,
                dt+75,dt+1000,
                5,10,40,30)
            assert.equal(await this.token.balanceOf(this.basic.address),30)
            await ethers.provider.send('evm_increaseTime', [50])
            await ethers.provider.send('evm_mine')
            await expectRevert(buy(this.basic,0,10), "More than MaxPerUser")
        })

        it("Test buyQuota More than MaxAmount.", async function () {
            let dt = await getDT()
            await testAddPair(this.basic,this.depToken1,dt+50,
                dt+75,dt+1000,
                100,10,40,30,10)
            ...
            await ethers.provider.send('evm_increaseTime', [50])
            await ethers.provider.send('evm_mine')
            await expectRevert(buy(this.basic,0,10), "More than MaxAmount")
        })
    })
    ...
    async function testAddPair(basic,token,startTime,endtime,withdrawtime,max,price,minAmount,salesAmount=10000,maxAmount=10000){
        await basic.addPair(token.address,salesAmount,startTime,endtime,withdrawtime,max,price,minAmount,maxAmount,{from:admin})
